...
<script>
  ...
  const addEvent = (target, type, handler) => {
    if (type === 'start') {
      target.addEventListener('mousedown', handler, supportsPassive ? { passive: true } : false);
      target.addEventListener('touchstart', handler, supportsPassive ? { passive: true } : false);
    } else {
      target.addEventListener('mouseup', handler, supportsPassive ? { passive: true } : false);
      target.addEventListener('touchend', handler, supportsPassive ? { passive: true } : false);
    }
  };
  ...
  const MdInkRipple = {
    ...
    computed: {
      ...
      disabled() {
        ...
        return this.mdDisabled || !this.$root.$material.inkRipple;
      }
    },
    watch: {
      disabled(disabled) {
