...
import tdlTypeaheadDialogTheme from './tdlTypeaheadDialog-theme.scss';
...
const TdlTypeaheadDialog = {
  ...
  computed: {
    cssProps() {
      return { '--vh': this.realViewportHeightScale };
    },
  ...
  created() {
    this.$root.$emit("component-created", { name: "tdlTypeaheadDialog", theme: tdlTypeaheadDialogTheme });
  },
  mounted() {
    this.$nextTick(() => {
  ...
  beforeDestroy() {
    if (this.$refs.inputDialog && this.$refs.inputDialog.parentNode &&
        this.$refs.inputDialog.parentNode === document.body) {
  ...
  methods: {
    ...
    itemClass(item) {
      if (this.defaultSelectableItem) {
        return {'results__result--selectable': this.defaultSelectableItem.id === item.id};
      }
      return {};
    },
    selectDefaultItem() {
