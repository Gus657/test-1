    ...
    <md-whiteframe md-tag="nav" class="md-steps-navigation" v-if="!mdVertical" :md-elevation="mdElevation" :class="navigationClasses" ref="stepNavigation">
      <md-step-header-container :md-vertical="mdVertical" ref="stepHeader">
        <md-step-header
          ...
          :class="{ 'no-hand-cursor': mdDisableClick }"
          @click.native="(!mdDisableClick) ? setActiveStep(step) : '' ">
        </md-step-header>
...
<script>
  ...
  import MdStepHeader from '@/lib-components/mdStepper/mdStepHeader.vue';
  import MdStepHeaderContainer from '@/lib-components/mdStepper/mdStepHeaderContainer.js';
  import mdStepperTheme from './mdStepper-theme.scss';
  ...
  const MdStepper = {
    ...
    created() {
      this.$root.$emit("component-created", { name: "mdStepper", theme: mdStepperTheme });
    },
    methods: {
      getNextStep(id) {
