...
var tdlTypeaheadListTheme = ".THEME_NAME.typeahead-list__item-list--active .md-highlighted .md-button:not([disabled]) {\n  background-color: BACKGROUND-INVERTED-0.12;\n}\n.THEME_NAME .typeahead-list__results-item--selectable {\n  background-color: ACCENT-PRIMARY-0.12;\n}\n.THEME_NAME.typeahead-list__search-hint {\n  color: BACKGROUND-INVERTED-0.30 !important;\n}\n.THEME_NAME.typeahead-list__item-list .typeahead-list__results-item.md-disabled {\n  color: TEXT-DISABLED;\n}";
...
const TdlTypeaheadList = {
  ...
  computed: {
    handleFocusChangeThrottled() {
      return throttle(this.handleFocusChange, 200, {
        trailing: false
      });
    },
  ...
  created() {
    this.$root.$emit("component-created", {
      name: "tdlTypeaheadList",
      theme: tdlTypeaheadListTheme
    });
  },
  ...
  beforeDestroy() {
    if (this.listElementRef && this.listElementRef.parentNode && this.listElementRef.parentNode === document.body) {
  ...
  methods: {
    ...
    onInputBlur() {
      ...
      if (!this.isInteractingWithList) {
        this.closeList();
        this.closeListOnItemSelected = false; // blur event might trigger other actions such as creating an item
    ...
    itemClass(item) {
      if (this.defaultSelectableItem !== undefined) {
        return {
          'typeahead-list__results-item--selectable': item.id === this.defaultSelectableItem.id
        };
      }
    },
    ...
    setListTopLeftHeight() {
      ...
      if (this.itemListOffset.maxWidth) {
        ...
        const parent = getClosestVueParent.getClosestVueParent(this.$parent, 'tdl-typeahead') || this.$parent;
      ...
      const listHeight = this.flattenItemList.length * listItemHeight + this.subtitleTotalItems * subtitleHeight; // doesn't fit in the screen below the input
      ...
      if (input.clientHeight + inputRect.top + listHeight > window.innerHeight - (listItemHeight + subtitleHeight)) {
        ...
        const requiredSubtitleHeight = subtitleHeight * Math.min(1, this.subtitleTotalItems); // can't fit at least 2 items + 1 subtitle
    ...
    enableOutsideClickListener() {
      this.outsideClickListener = document.addEventListener('click', event => {
        ...
        const clickInsideListItem = !!listItemRect && x >= listItemRect.x && x <= listItemRect.x + listItemRect.width && y >= listItemRect.y && y <= listItemRect.y + listItemRect.height;
        const inputRect = this.$el.getBoundingClientRect();
        const clickInsideInput = x >= inputRect.x && x <= inputRect.x + inputRect.width && y >= inputRect.y && y <= inputRect.y + inputRect.height;
    ...
    setListTopLeftHeightIfInputMoved(inputRect) {
      ...
      const rectChanged = currentRect.x !== inputRect.x || currentRect.y !== inputRect.y || currentRect.width !== inputRect.width || currentRect.height !== inputRect.height || currentRect.bottom !== inputRect.bottom || currentRect.left !== inputRect.left || currentRect.right !== inputRect.right || currentRect.top !== inputRect.top;
...
module.exports = TdlTypeaheadList;
